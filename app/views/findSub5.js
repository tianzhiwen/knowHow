...
export default class findSub5 extends Component {
    ...
    render() {
        ...
        return (
            <View style={{flex:1, backgroundColor:'#FAFAFA', padding: 4}}>
                <TitleBar title="趣动旅程" iconLeft={require('./../../images/goback.png')} backClick={()=>{
    ...
    _renderItem(i){
        return (
            <TouchableOpacity key={i.id} style={{backgroundColor:'#fff', width:Utils.size.width/2 - 8, marginTop:10, marginBottom:10}} onPress={()=>{
                                 nav.navigate('FindDetail',{title:'趣动旅程', src:{uri:Utils.findDetail + i.id}});
                            }}>
                <Image style={{width:Utils.size.width/2 - 8, height:100}} source={{uri:Utils.imageBaseUrl + "/" + i.cover}} />
                <Text>{i.heading}</Text>
            </TouchableOpacity>
        );
    }
    ...
    _getDataList(){
        ...
        Utils.get(Utils.baseNet + Utils.getFindTypeList + "zhishi_cmd_guess&pageNO=" + this.state.pageIndex + "&pageSize=" + this.state.pageSize,
            data=>{
                console.log(data);
                if (data.state == 1){
