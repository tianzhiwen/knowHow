...
export default class ScrollSelectView extends Component {
    ...
    render() {
        ...
        for (let i = start; i <= this.state.endNum; i++){
            let style = {
                color :'rgba(150, 216, 202,0.3)'
            };
            if ((i - 2) > 0 && (i - 3) == this.state.page){
                style.color = 'rgba(150, 216, 202,0.3)';
            }


            if ((i - 2) > 0 && (i - 2) == this.state.page){
                style.color = 'rgba(150, 216, 202,0.4)';
            }

            if ((i - 1) > 0 && (i - 1) == this.state.page){
                style.color = 'rgba(150, 216, 202,0.8)';
            }
        ...
        return (
                <View style={[{height:itemHeight * 5, width: this.props.width}]}>
                    ...
                    <View style={{position:'absolute',height:2 * Utils.pixel, width: this.props.width, backgroundColor:'rgb(150, 216, 202)', top:itemHeight * 2}}></View>

                    <View style={{position:'absolute',height:2 * Utils.pixel, width: this.props.width, backgroundColor:'rgb(150, 216, 202)', top:itemHeight * 3}}></View>
                </View>
        );
    }
